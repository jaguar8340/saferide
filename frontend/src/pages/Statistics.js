...
function Statistics() {
  ...
  const handleExportPDF = () => {
    ...
    doc.autoTable({ head: [['Einnahmen-Konten', 'Betrag']], body: incomeData, startY: 25 });
    ...
    doc.autoTable({ head: [['Ausgaben-Konten', 'Betrag']], body: expenseData, startY: doc.lastAutoTable.finalY + 10 });
  ...
  const monthlyChartData = Object.entries(statistics.monthly_data || {}).map(([monthKey, data]) => {
    ...
    return { month: monthNames[monthNum - 1] || monthNum, Einnahmen: data.income || 0, Ausgaben: data.expense || 0 };
  });
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
      ...
      <Card className="border-0 shadow-lg mb-6"><CardHeader><CardTitle>Monatliche Übersicht</CardTitle></CardHeader><CardContent><ResponsiveContainer width="100%" height={350}><BarChart data={monthlyChartData}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="month" /><YAxis /><Tooltip /><Legend /><Bar dataKey="Einnahmen" fill="#27ae60" /><Bar dataKey="Ausgaben" fill="#e67e22" /></BarChart></ResponsiveContainer></CardContent></Card>
