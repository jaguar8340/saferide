...
function CustomerManagement() {
  ...
  const handleDeleteRemark = async () => {
    ...
    try {
      ...
      Object.keys(remarks).forEach(cid => fetchRemarks(cid));
    } catch (error) {
      toast.error('Fehler beim Loeschen');
    } finally {
          ...
          <AccordionContent>
            <div className="px-6 pb-6 space-y-4">
              ...
              <div className="flex justify-end gap-2">
                <Button variant="outline" size="sm" onClick={() => { setEditingCustomer(c); setCustomerForm(c); setShowDialog(true); }}>
                  <Edit2 className="mr-2 h-4 w-4" />Bearbeiten
                </Button>
                <Button variant="outline" size="sm" onClick={() => setDeleteConfirm(c.id)} style={{ color: '#d63031' }}>
              ...
              <div className="border-t pt-4">
                ...
                <div className="space-y-3 mb-4">
                  <Textarea
                    ...
                    value={selectedCustomer === c.id ? remarkText : ''}
                    onChange={(e) => { setSelectedCustomer(c.id); setRemarkText(e.target.value); }}
                    rows={2}
                  />
                  <div className="flex gap-2">
                    <Input
                      ...
                      onChange={(e) => { setSelectedCustomer(c.id); setRemarkFile(e.target.files[0]); }}
                      className="flex-1"
                    />
                    <Button
                      onClick={() => handleAddRemark(c.id)}
                      disabled={!remarkText.trim() || selectedCustomer !== c.id}
                ...
                {remarks[c.id] && remarks[c.id].length > 0 ? (
                  ...
                  <p className="text-gray-500 text-sm">Noch keine Bemerkungen</p>
                )}
              </div>
            </div>
          </AccordionContent>
