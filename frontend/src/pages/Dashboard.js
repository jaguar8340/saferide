...
function Dashboard() {
  ...
  const fetchBankDocuments = async () => {
    try {
      ...
      toast.error('Fehler beim Laden der Bankbelege');
    }
  };
  ...
  const fetchMiscItems = async () => {
    try {
      ...
      toast.error('Fehler beim Laden der Diverses-Einträge');
    }
  };
...
function TransactionsTab({ transactions, handleEdit, setDeleteConfirm, handleFileUpload, uploadingFile, API }) {
  return (
...
// Component for Bank Documents Tab
function BankDocumentsTab({ bankDocuments, handleAddBankDocument, handleDeleteBankDocument, API }) {
  const [selectedFile, setSelectedFile] = useState(null);

  const handleUpload = () => {
    if (selectedFile) {
      handleAddBankDocument(selectedFile);
        ...
        <div className=\"flex justify-between items-center\">
          ...
          <div className=\"flex gap-2 items-center\">
            <Input
              ...
              className=\"max-w-xs\"
            />
            <Button onClick={handleUpload} disabled={!selectedFile} style={{ background: '#d63031', color: 'white' }}>
...
// Component for Misc Items Tab
function MiscItemsTab({ miscItems, handleAddMiscItem, handleDeleteMiscItem, API }) {
  const [remarks, setRemarks] = useState('');
  const [selectedFile, setSelectedFile] = useState(null);

  const handleAdd = () => {
    if (remarks.trim()) {
      handleAddMiscItem(remarks, selectedFile);
      setRemarks('');
      setSelectedFile(null);
    }
  };
        ...
        <div className=\"space-y-4 mb-6\">
          ...
          <div className=\"flex gap-2 items-center\">
            <Input
              ...
              className=\"flex-1\"
            />
            <Button onClick={handleAdd} disabled={!remarks.trim()} style={{ background: '#d63031', color: 'white' }}>
              <Plus className=\"mr-2 h-4 w-4\" />
              Hinzufügen
            </Button>
          </div>
